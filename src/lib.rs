...
mod test_events {
    ...
    fn include_text_notes_referencing_another_event_without_pubkeys() {
        ...
        let tags = [Tag::Event(
            event_id,
            Some(UncheckedUrl::from("wss://relay.example.com")),
            None,
        )];
        let e_note: Event = EventBuilder::new_text_note("Text note from nostr-sdk", &tags)
            .to_event(&my_keys)
            .unwrap();
    ...
    fn exclude_text_notes_referencing_pubkeys() {
        ...
        let tags = [Tag::PubKey(other_pub_key, None)];
        let e_note: Event = EventBuilder::new_text_note("Text note from nostr-sdk", &tags)
            .to_event(&my_keys)
            .unwrap();
